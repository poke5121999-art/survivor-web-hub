...
(function (G) {
  ...
  // nằm hai bên mốc +-pi cho ra một hiệu gần 2pi và mọi phép thử hình quạt sai.
  function ang2(a, b) { return ang(a - b); }
  ...
  Battle.prototype.setupPlayer = function () {
    ...
    this.player = {
      ...
      ulti: 0, swapCd: 0, swapCombo: 0, swapAt: 0,
  ...
  Battle.prototype.setWeapon = function (i, instant) {
    ...
    i = ((i % 3) + 3) % 3;
    /* HỒI CHIÊU ĐỔI NGƯỜI. `instant` là đường vào lúc bắt đầu ải và lúc nạp lại
     * hồ sơ — nó phải đi lọt, nếu không thì trận nào cũng mở màn bằng một lời từ
     * chối. Chỉ cú đổi do NGƯỜI CHƠI bấm mới bị tính giờ. */
    if (!instant) {
      if (i === this.player.wIdx) return;
      if ((this.player.swapCd || 0) > 0) {
        this.toast('Đổi người còn ' + ((this.player.swapCd / 1000).toFixed(1)) + 's', '#8fa3b5');
        return;
      }
      if (this.player.down || this.paused) return;
    }
    var fromEl = (this.wp && this.wp.el) || null;
    var h = party[i];
    ...
    if (!instant) {
      this.player.switchT = 420; this.player.state = 'switch';
      this.player.stateT = 0; this.player.stateDur = 420;
      this.player.swapCd = G.SWAP.cd;
      this.swapEntry(fromEl);
    }
    if (this.cb.onWeapon) this.cb.onWeapon(this.wp, i, h);
  };

  /* ================== ĐÒN RA MẮT KHI ĐỔI NGƯỜI =========================
   * Người mới lao ra kèm một cú nổ quanh chân. Ba tầng cộng dồn:
   *   nền      — sát thương theo hệ của người MỚI
   *   chuỗi    — đổi liên tiếp thì mạnh dần, thưởng cho việc xoay cả đội
   *   phản ứng — hệ cũ và hệ mới khắc nhau thì nổ to hơn và rộng hơn
   *
   * Cú nổ này KHÔNG tính vào thanh ulti (aoeDamage đi đường riêng, không qua
   * ultiGain của đòn thường). Cố ý: nếu nó nạp ulti thì đổi người liên tục là
   * cách nạp nhanh nhất, và cả hai hệ thống cùng sụp vào một nút bấm. */
  Battle.prototype.swapEntry = function (fromEl) {
    var p = this.player, S = G.SWAP;
    var now = this.t;
    // chuỗi: còn trong cửa sổ thì cộng nấc, hết cửa sổ thì về 1
    if (p.swapAt && now - p.swapAt <= S.comboMs) {
      p.swapCombo = Math.min(S.comboMax, (p.swapCombo || 1) + 1);
    } else {
      p.swapCombo = 1;
    }
    p.swapAt = now;

    var toEl = (this.wp && this.wp.el) || 'none';
    var react = G.reactOf(fromEl, toEl);
    var chain = 1 + (p.swapCombo - 1) * S.comboStep;
    var mul = S.entryMul * chain * (react ? S.reactMul : 1);
    var r = react ? S.reactR : S.entryR;

    var col = react ? react.col : (G.ELEMENTS[toEl] || G.ELEMENTS.none).color;
    this.fx.push({ k: 'ring', x: p.x, y: p.y, r: r, t: 0, ms: react ? 420 : 300, col: col });
    this.puff(p.x, p.y, col);
    this.shake = Math.max(this.shake, react ? 10 : 5);
    this.aoeDamage(p.x, p.y, r, mul, { el: toEl, status: react && react.st, from: p.facing });

    if (react) this.toast(react.n + (p.swapCombo > 1 ? ' ×' + p.swapCombo : ''), react.col);
    else if (p.swapCombo > 1) this.toast('ĐỔI NGƯỜI ×' + p.swapCombo, col);
  };
  ...
  /* VÒNG QUAY của súng quay: 0 khi vừa bóp cò, 1 khi quay hết. Nó ở trên
   * người chơi chứ không ở vũ khí, vì nó phải sống qua cả những khung mà cây
   * súng không bắn — và phải TUỘT trong những khung đó. */
  ...
  Battle.prototype.stepSpin = function (dt) {
    ...
    // Giữ cò (hoặc đang trong Bão Chì) thì lên, ngoài ra thì tuột — và tuột
    // nhanh hơn lên. Bất đối xứng là cố ý: đứng lại phải là một CAM KẾT.
    var up = (p.state === 'autofire' || p.state === 'fire' || p.spinLock > this.t);
    var k = dt / Math.max(1, W.spinMs);
    p.spin = clamp((p.spin || 0) + (up ? k : -k * (W.spinDecay || 2)), 0, 1);
  };

  Battle.prototype.atkSpeed = function () {
    var m = 1;
    // Súng quay: nhịp bắn nhân theo vòng quay, không phải một hằng số. Nhân ở
    // ĐÂY để mọi thứ đọc nhịp bắn (bắn, bot, bảng chỉ số) đều thấy cùng con số.
    if (this.W && this.W.mode === 'spin') {
      var sm = this.W.spinMul || [0.55, 2.3];
      m *= sm[0] + (sm[1] - sm[0]) * this.spinK();
    }
  ...
  Battle.prototype.holdStart = function (dx, dy) {
    ...
    /* NẠP ĐỦ THÌ GIỮ = NGẮM ULTI, chứ không phải đòn đặc thù của vũ khí.
     *
     * Đây là toàn bộ cách xả kỹ năng: không nút HUD, không ngón thứ hai. Đúng
     * ngữ pháp Punicon — giữ để vào thế, kéo để chỉ hướng, thả để ra đòn.
     *
     * Nhánh này phải nằm TRƯỚC nhánh vũ khí, vì hai thứ dùng chung một cử chỉ và
     * chỉ một cái được thắng. Thanh ulti là thứ phân xử: chưa đủ nạp thì giữ vẫn
     * là ghì súng / nạp cung / bắn liên tục như cũ, đủ nạp rồi thì giữ là ngắm
     * đòn lớn. Người chơi biết mình đang ở nhánh nào bằng cách nhìn thanh trên
     * đầu nhân vật — nó vàng rực khi qua nấc. */
    var armed = this.ultiArmed();
    if (armed >= 0 && this.skillAimStart(armed)) {
      p.state = 'ultiaim'; p.stateT = 0;
      // Đứng yên khi ngắm: kéo ngón lúc này là CHỈ HƯỚNG, không phải đi.
      this.puni.holdMoves = false;
      return true;
    }
  ...
  Battle.prototype.holdTick = function (ms, dx, dy) {
    var p = this.player, W = this.W;
    // Đang ngắm ulti: mọi thứ khác im lặng, ngón chỉ làm một việc là xoay hướng.
    if (p.state === 'ultiaim') { this.skillAimMove(dx, dy); return; }
    p.charge = ms;
  ...
  Battle.prototype.holdEnd = function (dx, dy, ms) {
    ...
    p.beamTicks = 0;
    if (p.state === 'ultiaim') {
      var idx = this.skillAiming();
      p.state = 'idle';
      // Trừ thanh CHỈ KHI đòn thật sự ra. skillAimEnd trả false khi bị từ chối
      // (vừa ngã, ải vừa dừng), và trừ trước thì người chơi mất trắng cả thanh
      // cho một đòn không bao giờ xảy ra.
      if (this.skillAimEnd() && idx >= 0) this.ultiSpend(idx);
      return;
    }
  ...
  Battle.prototype.roll = function (raw, target, opt) {
    ...
    // XÉ GIÁP. Đọc ở đây vì đây là chỗ DUY NHẤT giáp được dùng — trừ giáp ở chỗ
    // khác thì sẽ có một đường tính sát thương không biết tới vết hở.
    if (target && target.armorCut && target.armorCutUntil > this.t) {
      armor *= (1 - target.armorCut);
    }
  ...
  Battle.prototype.updateAction = function (dt, mv, slowMul) {
    ...
    switch (p.state) {
      ...
      /* NGẮM ULTI: đứng chôn chân, không bắn, không đi. Ngón đang chỉ hướng.
       * Đây là cái giá duy nhất của đòn lớn: một khoảnh khắc hở sườn hoàn toàn.
       * Không đặt giá đó thì xả ulti là hành động miễn phí, và mọi lựa chọn
       * "xả bây giờ hay nhịn thêm" đều mất nghĩa. */
      case 'ultiaim': {
        break;
      }
  ...
  Battle.prototype.updateProjectiles = function (dt) {
    ...
    for (var i = this.projs.length - 1; i >= 0; i--) {
      ...
      /* LUÂN XA QUAY ĐẦU. Hết outMs thì đích đổi thành NGƯỜI CHƠI — đọc lại vị
       * trí mỗi khung, nên đứng yên hay vừa chạy vừa ném cho ra hai đường về
       * khác hẳn nhau. hitSet xoá sạch lúc quay đầu: cùng một con phải ăn được
       * cả hai lượt, đó là toàn bộ điểm mạnh của lớp này. Bắt được về tay thì
       * đĩa tan — không phải để dọn dẹp, mà để đứng sát mặt không ăn ba lượt. */
      if (pr.boomerang) {
        pr.spin = (pr.spin || 0) + (this.W.spinDeg || 900) * dt / 1000;
        pr.bt += dt;
        if (!pr.back && pr.bt >= (this.W.outMs || 420)) {
          pr.back = true; pr.hitSet = []; pr.hits = 0;
          pr.mul *= (this.W.backMul === undefined ? 0.85 : this.W.backMul);
        }
        if (pr.back) {
          var pl = this.player;
          pr.a = Math.atan2(pl.y - pr.y, pl.x - pr.x);
          if (Math.hypot(pl.x - pr.x, pl.y - pr.y) < pl.r + pr.r) {
      ...
      /* ĐĨA NẢY. Đặt ngay trước phần xử lý va chạm để nó thấy được kết quả của
       * khung TRƯỚC: cờ `needBounce` do chỗ xử lý trúng bật lên, còn việc chọn
       * mục tiêu mới thì làm ở đây. Gộp cả hai vào chỗ xử lý trúng thì phải lặp
       * lại logic ấy ở ba nhánh (quái, boss, bộ phận boss).
       *
       * Chọn con gần nhất CHƯA trúng: không loại trừ thì đĩa nảy đi nảy lại giữa
       * đúng hai con và mười hai lượt nảy thành sáu lượt. */
      if (pr.needBounce) {
        pr.needBounce = false;
        var bt = null, bd2 = pr.bounceR || 260;
        for (var bi = 0; bi < this.mobs.length; bi++) {
          var bm = this.mobs[bi];
          if (bm.hp <= 0 || pr.hitSet.indexOf(bm) >= 0) continue;
          var bdd = Math.hypot(bm.x - pr.x, bm.y - pr.y);
          if (bdd < bd2) { bd2 = bdd; bt = bm; }
        }
        if (bt && pr.bounceLeft > 0) {
          pr.bounceLeft--;
          pr.a = Math.atan2(bt.y - pr.y, bt.x - pr.x);
          pr.mul *= (1 - (pr.bounceFall || 0.07));
          this.fx.push({ k: 'ring', x: pr.x, y: pr.y, r: 16, t: 0, ms: 200,
                         col: this.elemFx().col });
        } else {
          this.projs.splice(i, 1); continue;
        }
      ...
      if (hitSomething) {
        ...
        /* ĐĨA NẢY đi trước luật xuyên: nó KHÔNG xuyên (pierce = false) nhưng
         * cũng không tan — nó đổi hướng. Bật cờ rồi để khung sau chọn mục tiêu,
         * vì lúc này ta còn đang ở giữa vòng lặp va chạm và đổi hướng ngay đây
         * thì viên đạn tự trúng lại chính con vừa trúng trong cùng một khung. */
        if (pr.bounceLeft > 0) { pr.needBounce = true; continue; }
   ...
   * là thứ duy nhất phân biệt cây này với mọi cây cận chiến khác, nên nó nằm
   * ngay đây chứ không phải một tuỳ chọn ở đâu đó. */
  Battle.prototype.fireArc = function (base) {
    var p = this.player, W = this.W, self = this;
    var a0 = p.facing, half = W.arcSpan / 2, len = W.arcLen;
    this.mobs.forEach(function (m) {
      if (m.hp <= 0) return;
      var dx = m.x - p.x, dy = m.y - p.y, d = Math.hypot(dx, dy);
      if (d > len + m.r || d < 1) return;
      if (Math.abs(ang2(Math.atan2(dy, dx), a0)) > half) return;
      self.dealToMob(m, self.playerDamage(base, {}),
        { move: { kb: 0, poise: W.poise, hs: W.hs }, from: a0 });
      // GIẬT VỀ: không đẩy ra. Trừ vào vector hướng tới nạn nhân, và chặn lại
      // trước khi nó chui vào trong người mình.
      var pull = Math.min(W.pull, Math.max(0, d - (m.r + 18)));
    ...
    if (this.boss && this.boss.hp > 0) {
      ...
      if (Math.hypot(bx, by) < len + this.boss.r &&
        ...
        this.dealToBoss(this.playerDamage(base, {}),
          p.x + Math.cos(a0) * len * 0.6, p.y + Math.sin(a0) * len * 0.6,
          { move: { hs: W.hs } });
      }
    }
    this.fx.push({ k: 'whiparc', x: p.x, y: p.y, a: a0, len: len, half: half,
                   t: 0, ms: 190, col: this.elemFx().col });
  };

  /* ---- LUÂN XA: một viên đạn có ĐƯỜNG VỀ. `back` bật lên khi hết outMs, và
   * từ lúc đó đích của nó là NGƯỜI CHƠI ở vị trí HIỆN TẠI — không phải chỗ đã
   * ném đi. Đó là chỗ cây này có chiều sâu: vừa ném vừa chạy thì đường về quét
   * một hàng khác hẳn đường đi. hitSet được xoá khi quay đầu, nên cùng một con
   * ăn được cả hai lượt. */
  Battle.prototype.fireBoomerang = function (base, spread, crit) {
    var p = this.player, W = this.W;
    var off = (Math.random() - 0.5) * spread * Math.PI / 180;
    this.projs.push({
      k: 'shot', wclass: W.id, boomerang: true, back: false, bt: 0,
      x: p.x + Math.cos(p.facing) * 14, y: p.y + Math.sin(p.facing) * 14,
      a: p.facing + off, spd: W.spd, life: W.life, r: W.r,
      mul: base, critBonus: crit,
      pierce: true, pierceMax: 0, pierceFall: W.pierceFall || 0.18, hits: 0,
      from: { x: p.x, y: p.y }, hitSet: [], fade: G.DANMAKU.fadeInMs,
      spin: 0, life0: W.life
    });
  };

  /* ---- TƯỜNG LỬA: một dải cháy nằm trên mặt đất, sống độc lập với người chơi.
   * Nó KHÔNG đuổi theo ai và không tự tìm mục tiêu — đó chính là điều làm nó
   * khác mọi đòn khác: giá trị của nó là bắt kẻ địch phải chọn đường, chứ không
   * phải là con số nó gây ra. Đánh theo NHỊP (tickMs) chứ không mỗi khung, nếu
   * không thì đứng trong lửa một giây là chết ngay và cả đòn thành cái bẫy chết
   * người thay vì một bức tường. */
  Battle.prototype.stepLanes = function (dt) {
    if (!this.lanes || !this.lanes.length) return;
    var self = this;
    for (var i = this.lanes.length - 1; i >= 0; i--) {
      var L = this.lanes[i];
      L.left -= dt; L.tick += dt;
      if (L.left <= 0) { this.lanes.splice(i, 1); continue; }
      while (L.tick >= L.tickMs) {
        L.tick -= L.tickMs;
        var vx = Math.cos(L.a), vy = Math.sin(L.a), half = L.len / 2;
        this.mobs.forEach(function (m) {
          if (m.hp <= 0) return;
          var rx = m.x - L.x, ry = m.y - L.y;
          var along = rx * vx + ry * vy;
          if (Math.abs(along) > half + m.r) return;
          if (Math.abs(rx * -vy + ry * vx) > L.w / 2 + m.r) return;
          self.dealToMob(m, self.playerDamage(L.mul, { el: 'fire' }),
            { move: { kb: 0, poise: 4, hs: G.FEEL.hitstop.tick }, from: L.a, skill: true });
          if (L.burn) self.applyStatus(m, 'burn');
        });
      }
    }
  };

  Battle.prototype.drawLanes = function () {
    if (!this.lanes || !this.lanes.length) return;
    var ctx = this.ctx;
    for (var i = 0; i < this.lanes.length; i++) {
      var L = this.lanes[i];
      // Nhấp nháy theo thời gian còn lại: hai giây cuối nó nhạt dần, nên người
      // chơi biết bức tường sắp hết mà tính đường đi trước.
      var k = Math.min(1, L.left / 1800);
      ctx.save();
      ctx.translate(L.x, L.y); ctx.rotate(L.a);
      ctx.globalCompositeOperation = 'lighter';
      for (var j = 0; j < 3; j++) {
        var w = L.w * (1 - j * 0.28) * (0.9 + 0.1 * Math.sin(this.t / 90 + j));
        ctx.globalAlpha = (0.20 + j * 0.16) * k;
        ctx.fillStyle = j === 2 ? '#fff2c0' : (L.col || '#ff7a3c');
        ctx.fillRect(-L.len / 2, -w / 2, L.len, w);
      }
      ctx.restore();
    }
  };
  ...
  /* ====================== THANH ULTI: NẠP / ĐỌC / TRỪ =====================
   * Một cú TRÚNG cộng bao nhiêu? Đủ để bắn trúng liên tục `G.ULTI.sec` giây thì
   * đầy — với mọi lớp. Nên mẫu số là số cú trúng mà cây đó ra được trong ngần ấy
   * giây: (phát/giây) x (viên mỗi phát) x giây.
   *
   * Chia cho SỐ VIÊN là chỗ dễ quên nhất. Không chia thì súng săn bắn sáu viên
   * một phát nạp nhanh gấp sáu lần súng trường, và cả bảng cân bằng đổ. Chia rồi
   * thì súng săn trúng trọn sáu viên = một phát của súng trường, đúng như nó
   * đáng được. Bắn hụt vài viên thì nạp chậm lại — đó là ý đồ, không phải lỗi. */
  Battle.prototype.ultiPerHit = function () {
    var W = this.W;
    var shotsPerSec = Math.max(0.2, (W.rpm || 120) / 60);
    var perShot = Math.max(1, W.shots || 1);
  ...
  /* Kỹ năng nào đang xả được? Trả về CHỈ SỐ, hoặc -1 nếu chưa đủ nạp.
   *
   * Ưu tiên đòn LỚN khi thanh đã đầy: người chơi nhịn tới đầy thanh là để lấy
   * đòn lớn, nên nếu lúc đó vẫn xả đòn nhỏ thì cả việc nhịn thành vô nghĩa. Chỉ
   * khi đòn lớn chưa mở (dưới Lv.8) hay đang khoá thì mới rơi về đòn nhỏ. */
  Battle.prototype.ultiArmed = function () {
    var u = this.player.ulti || 0;
    if (u >= 0.999 && this.skillReady(1)) return 1;
    if (u >= G.ULTI.notch && this.skillReady(0)) return 0;
    return -1;
  };
  ...
  Battle.prototype.drawPlayerPlate = function (p) {
    ...
    /* ---- THANH ULTI TRÊN ĐẦU, ngay dưới thanh máu ----
     * Đây là thứ người chơi nhìn nhiều thứ hai sau thanh máu, nên nó phải đọc
     * được bằng mắt ngoại vi — trong lúc mắt chính đang bận nhìn đám quái.
     *
     * Ba trạng thái, ba tín hiệu khác HẠNG chứ không khác sắc độ:
     *   chưa tới nấc  -> xanh lơ, im
     *   qua nấc       -> vàng, có vạch nấc rõ  (xả được đòn nhỏ)
     *   đầy           -> trắng, viền nháy      (xả được đòn lớn)
     * Vạch NẤC luôn hiện, kể cả lúc thanh còn cạn: nó là cái đích, mà một cái
     * đích chỉ hiện ra khi đã tới thì không dẫn đường được cho ai.
     *
     * Nạp vũ khí (cung, ghì súng) vẫn dùng chung chỗ này khi nó đang chạy — hai
    ...
    /* ---- TÊN ĐÒN SẮP XẢ, ngay trên thanh máu ----
     * Thay cho hai đĩa hồi chiêu cũ. Hồi chiêu không còn là thứ quyết định —
     * thanh nạp mới là — nên hai cái đĩa đếm ngược chỉ còn là nhiễu. Cái người
     * chơi cần biết lúc thanh sáng lên là: GIỮ NGÓN BÂY GIỜ THÌ RA ĐÒN GÌ. */
    var armed = this.ultiArmed();
    if (armed >= 0) {
   ...
   * ==================================================================== */
  /* Đĩa luân xa: một vành khuyết QUAY, không phải một viên tròn. Phải nhìn ra
   * nó đang xoay thì mới đọc được rằng nó sẽ quay về — một chấm tròn im lìm
   * trông y hệt mọi viên đạn khác. Lượt về vẽ trắng và có quầng, để phân biệt
   * được hai lượt trên cùng một đường bay. */
  Battle.prototype.drawChakram = function (pr, col) {
    var ctx = this.ctx, R = pr.r + 5;
    ctx.save(); ctx.translate(pr.x, pr.y);
    if (pr.fade > 0) ctx.globalAlpha = 0.42;
    if (pr.back) {
      ctx.beginPath(); ctx.arc(0, 0, R + 5, 0, TAU);
      ctx.fillStyle = 'rgba(255,255,255,.12)'; ctx.fill();
    }
    ctx.rotate((pr.spin || 0) * Math.PI / 180);
    for (var i = 0; i < 3; i++) {
      var a0 = i / 3 * TAU;
      ctx.beginPath();
      ctx.arc(0, 0, R, a0, a0 + 0.72);
      ctx.arc(0, 0, R * 0.52, a0 + 0.72, a0, true);
      ctx.closePath();
      ctx.fillStyle = pr.back ? '#ffffff' : col;
      ctx.fill();
    }
    ctx.beginPath(); ctx.arc(0, 0, R * 0.34, 0, TAU);
    ctx.fillStyle = 'rgba(12,18,26,.85)'; ctx.fill();
    ctx.restore();
  };
  ...
  Battle.prototype.drawProjectiles = function () {
    ...
    this.projs.forEach(function (p) {
      ...
      if (p.boomerang)     { self.drawChakram(p, pcol); return; }
