/*
 * lai.js — lái Chrome headless bằng CDP để tự chơi thử và chụp màn hình.
 *
 * Gốc là `games/deepcore/_tools/drive.js` của game khác trong repo; chép sang đây thay vì gọi
 * chéo thư mục, vì mỗi game trong repo tự giữ bộ _tools của mình — và không nên phụ thuộc vào
 * tệp mà agent khác có quyền sửa.
 *
 * Khác bản gốc hai chỗ: khung mặc định **1280×760 nằm ngang** (Ghế Nóng chỉ chơi ngang), và
 * `mobile:false` + `deviceScaleFactor:1` để ảnh chụp đúng cỡ điểm ảnh thật.
 *
 * DÙNG:
 *   node _tools/lai.js <url> <ảnh-ra.png> [số-giây] [--do="js"] [--dofile=tệp.js] [--size=WxH]
 *
 * In mọi lỗi và console.* của trang rồi chụp màn hình. Có lỗi thì thoát mã 1, dùng được trong
 * kiểm thử tự động.
 *
 * Kịch bản tự chơi hết một mùa nằm ở `_tools/tuchoi.js`, nạp bằng `--dofile`.
 */
...
const sizeArg = (process.argv.find(a => a.startsWith('--size=')) || '--size=1280x760').slice(7);
...
(async () => {
  const profile = require('os').tmpdir() + '/gn-chrome-' + process.pid;
