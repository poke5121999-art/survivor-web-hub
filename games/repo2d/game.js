...
const CUT_ARRIVE = 3.25, CUT_DEPART = 2.65;
...
// KHÚC THOÁT. Chủ dự án, 2026-09-10: "khúc thoát thì cho bot chạy lên xe cùng player chạy đi
// mất." Bản trước hút cả tổ vào thùng xe trong 0,28 giây bằng một cú nội suy thẳng — không ai
// CHẠY cả, bốn cái bóng trôi ngang qua sàn rồi tắt. Nay nó là câu chuyện lúc tới kể ngược:
// chạy tới cửa sau, leo lên, cửa đóng, rồi xe vọt ra bằng ĐÚNG cái lỗ nó húc vào lúc tới.
const CUT_LEN_CHAY  = 0.60;                     // một người chạy từ chỗ đứng tới ghế mất ngần này
const CUT_LEN_LECH  = 0.10;                     // và người sau bắt đầu chậm hơn người trước
const CUT_LEN_DAU   = 0.05;                     // người đầu tiên nhấc chân lúc này
const CUT_DOOR_SHUT = [1.10, 1.50];             // leaving: doors first
const CUT_DRIVE_OFF = [1.55, 2.60];             // then the van
...
function startCut(kind, label, sub, then){
  ...
  S.cut = { kind, t:0, label:label||'', sub:sub||'', then:then||null, banged:false, shut:false,
            ...
            // Cả lúc ĐI cũng cần: xe ra bằng đúng cái lỗ nó vào, mà trạm dịch vụ thì không
            // có lỗ nào — ở đó nó lùi ngang ra như bản cũ.
            huc: xeDiemHuc(), gach: [], bui: [] };
...
function stepCut(dt){
  ...
  if (c.kind === 'arrive'){
    ...
    if (!c.dong && c.t >= CUT_DOOR_SHUT[0]){ c.dong = true; SFX.hinge(false); }
    if (!c.shut && c.t >= CUT_DOOR_SHUT[1]){ c.shut = true; SFX.thud(); }
    if (!c.no   && c.t >= CUT_DRIVE_OFF[0]){ c.no = true; SFX.engine(); }
    fxTheoGioThat(dt);
    // Máy quay đứng yên trên người chơi cho xe VỌT RA KHỎI KHUNG — bám theo thì nó cứ nằm
    // giữa màn và không đi đâu cả. Vẫn phải gọi mỗi khung: step() không chạy, mà camera thì
    // chỉ nhúc nhích trong step().
    camSnap();
...
function carDrawOffset(){
  ...
  const hucRa = c.huc || xeDiemHuc();
  const door = 1 - span(c.t, CUT_DOOR_SHUT[0], CUT_DOOR_SHUT[1]);
  // ĐI THÌ TĂNG TỐC, không phải hãm. easeOutCubic (thứ bản cũ dùng cho cả hai chiều) nghĩa là
  // vọt ngay ở khung đầu rồi chậm dần — đọc ra là xe đang PHANH. Bình phương thì ngược lại:
  // nhích một cái rồi mới lao, và ra tới mép khung thì nhanh gấp đôi tốc độ trung bình.
  const u = span(c.t, CUT_DRIVE_OFF[0], CUT_DRIVE_OFF[1]), k = u*u;
  if (!hucRa.tuong) return { dx: run*k, dy:0, rot:0, door, alpha:1 };
  // Ra bằng chính cái lỗ đã húc vào: quay mũi lên trước, rồi vọt thẳng lên và ra khỏi mép
  // trên khung. Quãng đi = từ chỗ đậu tới mặt tường, cộng nửa khung nữa cho khuất hẳn.
  const xa = (S.car.y - hucRa.y) + vwH()*0.65;
  return { dx: 0, dy: -xa*k, door, alpha: 1,
           rot: -Math.PI/2 * ease(span(c.t, CUT_DRIVE_OFF[0] - 0.20, CUT_DRIVE_OFF[0] + 0.30)) };
}

// ---------------------------------------------------------------- ai ngồi chỗ nào trên thùng xe
// SÁU chỗ, đo trong hệ toạ độ CỦA CHIẾC XE (x dọc thân, mũi ở +x; y ngang thân). Cả sáu nằm
// gọn trong thùng hàng: thùng chiếm từ -L/2 tới L/2 - cabL, tức -0,50L tới +0,20L. SÁU chứ
// không bốn vì Ca Trực Đêm đi bốn người (MATE_COUNT = 3) nhưng Biệt Đội đi NĂM (`SQ.squadList`
// trả một người dẫn + bốn bot) — bốn ghế thì người thứ năm ngồi đè lên người thứ hai.
//
// CÁCH XẾP ĐỔI THEO THẾ CỦA XE, và đây là chỗ dễ tưởng là làm màu. Hình người vẽ ĐỨNG THẲNG,
// cao 38 đơn vị, neo ở chân, mà chỉ rộng chừng 19. Hai người cách nhau 23 đơn vị theo chiều
// DỌC màn hình thì người sau chỉ còn hở mỗi cái đầu; cách nhau chừng ấy theo chiều NGANG thì
// rời hẳn nhau. Đo được ở khúc thoát bản đầu: bốn người leo lên thùng, trên ảnh chụp đếm ra
// HAI — hai cặp chồng lên nhau.
//
// Nên luật là: LUÔN GIÃN NGƯỜI THEO CHIỀU NGANG MÀN HÌNH.
//   — xe dựng (lúc lao vào nhà): ngang màn hình là BỀ RỘNG thùng  -> hai cột, ba hàng.
//   — xe nằm  (lúc đậu, lúc đi): ngang màn hình là CHIỀU DÀI thùng -> một hàng, sáu chỗ,
//     so le nhẹ lên xuống để còn đọc ra từng người.
// Hai cách ấy được TRỘN theo |sin(góc xe)| chứ không đổi phắt ở một mốc: xe quay từ thế chúi
// mũi về thế nằm ngay giữa lúc cả tổ còn ngồi trên thùng, mà đổi phắt thì cả bốn người nhảy
// sang chỗ mới trong một khung hình.
// `n` là TỔ CÓ MẤY NGƯỜI, và nó ở đây để cả tổ nằm GIỮA thùng chứ không dồn về đuôi: xếp
// cứng sáu chỗ rồi chỉ dùng bốn thì bốn người ấy chụm hết ở mép sau, còn nửa thùng trước bỏ
// không. Giãn cách cũng theo `n` — bốn người thì rộng tay, sáu người thì đứng sát lại.
function gheXeLocal(i, dung, n){
  // Tâm thùng hàng, tính TRONG HÀM chứ không thành một hằng ở đầu tệp: TRUCK_L khai báo mãi
  // dưới phần vẽ, mà một `const` ở đây đọc nó lúc nạp tệp thì rơi vào vùng chết của `const` —
  // ReferenceError ngay dòng đầu, cả game.js chết theo, và triệu chứng chỉ là "REPO is not
  // defined". Trong hàm thì nó chỉ chạy lúc được gọi, tức là sau khi cả tệp đã nạp xong.
  const giua = -TRUCK_L*0.15;
  if (dung){
    const hang = Math.max(1, Math.ceil(n/2));
    const c = i % 2, r = Math.min((i/2)|0, hang-1);
    return { lx: giua + (r - (hang-1)/2)*TRUCK_L*0.18, ly: (c - 0.5)*TRUCK_W*0.44 };
  }
  const cach = Math.min(TRUCK_L*0.16, TRUCK_L*0.62/Math.max(1, n-1));
  const k = i % XE_GHE_N;
  return { lx: giua + (k - (n-1)/2)*cach, ly: (k & 1) ? TRUCK_W*0.10 : -TRUCK_W*0.10 };
}
const XE_GHE_N = 6;                     // ghế 0 của người chơi, 1..5 cho bot
...
function gheXe(i, off, t){
  ...
  const dung = Math.min(1, Math.abs(sn)*1.4);          // 1 = xe dựng, 0 = xe nằm
  const n = clamp(1 + ((S.mates || []).length), 1, XE_GHE_N);
  const a = gheXeLocal(i % XE_GHE_N, true, n), b = gheXeLocal(i % XE_GHE_N, false, n);
  const lx = mix(b.lx, a.lx, dung), ly = mix(b.ly, a.ly, dung);
  // Trước cú húc: nhún đều. Sau: hết nhún, nhưng bị NÉM VỀ PHÍA MŨI XE một nhịp rồi dội lại.
  let nhun = 0, dua = 0;
  const vao = !S.cut || S.cut.kind === 'arrive';
  if (!vao) nhun = Math.sin(t*15 + i*1.9) * 1.4;   // lúc đi: xe nổ máy đứng chờ, nhún nhẹ hơn
  else if (t < CUT_HIT) nhun = Math.sin(t*15 + i*1.9) * 2.4;
  else {
...
// CHẠY LÊN XE — câu chuyện của xuongXe() kể ngược, và là cả yêu cầu "cho bot chạy lên xe
// cùng player". Ba chặng: đứng chờ tới lượt -> chạy tới miệng cửa sau -> leo lên ghế. Rồi cửa
// đóng và họ mờ đi: cái thùng kín rồi thì không còn nhìn thấy ai ngồi trong nữa.
//
// Chặng chạy nội suy THẲNG (không ease): một cú chạy có tốc độ đều, còn ease-out đọc ra là
// người ta lao ra rồi hãm lại ngay trước cửa xe.
function lenXe(i, a, off, t, mo){
  // Cửa đóng tới đâu thì người mờ tới đó, và mờ xong TRƯỚC lúc cửa khép hẳn.
  const kin = mo * (1 - span(t, CUT_DOOR_SHUT[0], mix(CUT_DOOR_SHUT[0], CUT_DOOR_SHUT[1], 0.7)));
  if (kin <= 0.01) return null;
  const t0 = CUT_LEN_DAU + i*CUT_LEN_LECH;
  const k = span(t, t0, t0 + CUT_LEN_CHAY);
  const ghe = gheXe(i, off, t);
  if (k >= 1) return { x: ghe.x, y: ghe.y, alpha: kin };
  const bat = choChay(a);
  if (k <= 0) return { x: bat.x, y: bat.y, alpha: kin };
  const cs = Math.cos(off.rot), sn = Math.sin(off.rot);
  const mieng = { x: S.car.x + off.dx - cs*(TRUCK_L*0.5 + 8),
                  y: S.car.y + off.dy - sn*(TRUCK_L*0.5 + 8) };
  if (k < 0.72){
    const u = k/0.72;
    return { x: mix(bat.x, mieng.x, u), y: mix(bat.y, mieng.y, u), alpha: kin };
  }
  const u = ease((k - 0.72)/0.28);
  return { x: mix(mieng.x, ghe.x, u), y: mix(mieng.y, ghe.y, u), alpha: kin };
}
// Chỗ một người BẮT ĐẦU chạy. Ai đang đứng trong khung thì chạy từ đúng chỗ mình đứng; ai ở
// xa hơn thì kéo về sát ngoài mép khung — chứ KHÔNG kéo về một vòng cố định quanh xe, vì làm
// thế là họ hiện ra giữa màn hình từ hư không. Kéo ra ngoài mép thì mắt đọc ra là "vừa chạy
// vào khung", và đó cũng gần đúng: hết ca thì đồng đội vốn đã tự tìm đường về xe (xem
// stepMates), nên chuyện phải kéo là chuyện hiếm.
function choChay(a){
  const bx = vwW()*0.5 + TILE, by = vwH()*0.5 + TILE;
  return { x: S.car.x + clamp(a.x - S.car.x, -bx, bx),
           y: S.car.y + clamp(a.y - S.car.y, -by, by) };
}
...
function mateDrawPos(a){
  ...
  const ghe = 1 + (((a.id|0) % (XE_GHE_N - 1)));
  return c.kind === 'arrive' ? xuongXe(ghe, a, off, c.t, mo) : lenXe(ghe, a, off, c.t, mo);
}
...
function playerDrawPos(){
  ...
  // Lúc đi thì ngược lại: chạy tới cửa sau rồi leo lên, cùng lượt với mấy con bot.
  return c.kind === 'arrive' ? xuongXe(0, p, off, c.t, mo) : lenXe(0, p, off, c.t, mo);
}
...
function drawMates(c){
  ...
  for (const a of S.mates){
    ...
    const skin = window.REPO_SKIN && REPO_SKIN.crew(c, S.cut ? voVe(a, cho.x, cho.y) : a, false);
...
function mateLights(c){
  ...
  for (const a of S.mates){
    ...
    // Trong đoạn phim thì cái đèn phải đi theo CHỖ VẼ. Toạ độ thật của họ đứng yên suốt đoạn
    // phim, nên bốn cái đèn nằm lại giữa sàn trong khi bốn người chạy về xe — mà đèn pin thì
    // cầm trên tay. Đây cũng là thứ làm cú chạy ĐỌC ĐƯỢC: trong nhà tối, cái đi động là ánh
    // sáng chứ không phải cái bóng.
    const q = S.cut ? mateDrawPos(a) : null;
    if (S.cut && !q) continue;
    const lx = q ? q.x : a.x, ly = q ? q.y : a.y;
    const r = 3.2*TILE;
    const g = c.createRadialGradient(lx, ly, 2, lx, ly, r);
    g.addColorStop(0, 'rgba(226,214,180,0.34)');
    g.addColorStop(1, 'rgba(226,214,180,0)');
    c.fillStyle = g; c.fillRect(lx-r, ly-r, r*2, r*2);
  }
}
...
function drawHeadGlow(c){
  ...
  for (const a of (S.mates || [])){
    if (a.down) continue;
    // CÁI VIỀN CŨNG PHẢI ĐI THEO NGƯỜI. Nó vẽ ở toạ độ THẬT, mà trong đoạn phim thì toạ độ
    // thật đứng yên — nên trước bản này, suốt cảnh xe tới và cảnh xe đi có mấy cái viền người
    // xanh lơ đứng chôn chân quanh chỗ đậu trong khi chính mấy người ấy đang ngồi trên xe.
    // Đo bằng ảnh chụp khúc thoát: một cái viền trắng đứng giữa sàn, không có ai bên trong.
    const q = S.cut ? mateDrawPos(a) : null;
    if (S.cut && !q) continue;
    const hx = q ? q.x : a.x, hy = q ? q.y : a.y;
    if (!inSight(hx, hy)) continue;
    ...
    c.restore();
    if (!veDuoc) glowRing(c, hx, hy, 13, [150,200,235], 0.28, 1.6);
  }
}
...
function buildLight(){
  ...
  // ĐÈN CỦA NGƯỜI CHƠI MỜ ĐI TRONG ĐOẠN PHIM, và nó không phải chuyện thẩm mỹ:
  //   — Lúc TỚI: người chơi đang ngồi trên xe ngoài đường, nhưng toạ độ THẬT của họ đã nằm sẵn
  //     ở chỗ đậu trong nhà. Để nguyên thì căn phòng sáng sẵn từ trước khi xe húc vào, và cú
  //     "đèn pha tràn vào qua lỗ thủng" chẳng mở ra được gì — nó rọi vào một chỗ đã sáng.
  //   — Lúc ĐI: cả tổ lên xe rồi mà cái đèn vẫn nằm lại giữa sàn soi cho một căn nhà không còn
  //     ai. Nhà phải TỐI LẠI sau lưng, đó là câu cuối của cả ca trực.
  // Không tắt hẳn: đa giác tầm nhìn vẫn tính từ chỗ ấy, và một khung hình đen thui thì đọc ra
  // là mất hình chứ không đọc ra là "đèn đã lên xe".
  const nguoiMo = denNguoiMo();
  const alphaCu = c.globalAlpha;
  if (nguoiMo < 1) c.globalAlpha = alphaCu * nguoiMo;
  ...
  c.globalAlpha = alphaCu;          // đèn XE ở dưới không được ăn theo cú mờ của đèn NGƯỜI
...
function drawDuongVao(c){
  const cut = S.cut;
  if (!cut || !(cut.huc && cut.huc.tuong)) return;
  const mo = denXeMo(cut);
  if (mo <= 0.01) return;
...
// Độ sáng của đèn pha, và cũng là độ đậm của con đường ngoài nhà: hai thứ ấy sống chết cùng
// nhau, vì con đường chỉ nhìn thấy được nhờ đèn xe.
//   VÀO — sáng từ khung đầu, tắt dần trong lúc cả tổ bước xuống.
//   ĐI  — tối cho tới lúc cửa đóng, rồi bật lên đúng lúc xe chuẩn bị vọt. Bật sớm hơn thì cả
//         khúc "chạy lên xe" bị rọi sáng trưng, mà khúc ấy vốn thuộc về căn nhà tối.
// Đèn của người chơi còn bao nhiêu phần. Xem chú thích ở chỗ dùng, trong buildLight.
const DEN_NGUOI_DAY = 0.12;
function denNguoiMo(){
  const cut = S.cut;
  if (!cut) return 1;
  if (cut.kind === 'arrive') return mix(DEN_NGUOI_DAY, 1, span(cut.t, CUT_HIT, CUT_STEP_OUT[1]));
  return mix(1, DEN_NGUOI_DAY, span(cut.t, CUT_LEN_DAU, CUT_DOOR_SHUT[1]));
}
function denXeMo(cut){
  if (!cut) return 0;
  return cut.kind === 'arrive' ? 1 - span(cut.t, CUT_STEP_OUT[0], CUT_ARRIVE - 0.15)
                               : span(cut.t, CUT_DOOR_SHUT[0], CUT_DRIVE_OFF[0]);
}
// ĐÈN THÙNG: một quầng ấm ở MIỆNG CỬA SAU, sống đúng quãng cửa còn mở.
//
// Nó không phải đồ trang trí. Đèn pha rọi về phía TRƯỚC, mà cả hai việc đáng nhìn — bước
// xuống lúc tới, chạy lên lúc đi — đều diễn ra ở phía SAU xe. Đo bằng ảnh chụp khúc thoát:
// không có nó thì trong bốn người leo lên xe chỉ đếm được hai, hai người kia ngồi trong một
// cái thùng màu than giữa một căn nhà tối.
function denThungXe(c, cut, off, mo){
  if (mo <= 0.01) return;
  const cs = Math.cos(off.rot), sn = Math.sin(off.rot);
  const x = S.car.x + off.dx - cs*TRUCK_L*0.34, y = S.car.y + off.dy - sn*TRUCK_L*0.34;
  const r = TILE*3.0;
  const g = c.createRadialGradient(x, y, 2, x, y, r);
  g.addColorStop(0, `rgba(236,214,168,${0.66*mo})`);
  g.addColorStop(0.55, `rgba(206,186,146,${0.30*mo})`);
...
function denXeVao(c){
  const cut = S.cut;
  if (!cut) return;
  const off0 = carDrawOffset();
  // Đèn thùng chạy theo CỬA, không theo đèn pha: mở tới đâu sáng tới đó.
  denThungXe(c, cut, off0, off0.door);
  const mo = denXeMo(cut);
  if (mo <= 0.01) return;
  const off = off0;
  const x = S.car.x + off.dx, y = S.car.y + off.dy;
...
function drawFoeOne(c, m, dIn){
  {
    // `dIn`: bảng chỉ số ĐƯA TỪ NGOÀI VÀO, cho sổ tay của Biệt Đội. Quái bên đó nằm trong
    // SQ.FOES chứ không nằm trong MONSTERS, nên MONSTERS[m.type] trả về undefined và cả hàm
    // này nổ ở dòng `d.noMelee`. Truyền thẳng def vào là cách rẻ nhất để một hàm vẽ dùng
    // được cho cả hai bảng quái, mà không phải trộn hai bảng lại làm một.
    const d = dIn || MONSTERS[m.type], s = Math.sin(m.wob)*1.5;
    ...
    // Ai KHÔNG mọc tay là chuyện của bảng chỉ số, không phải của cái mã loài. Trước đây câu
    // này viết thẳng 'm.type !== rook' — đúng cho Kẻ húc bên Ca Trực Đêm, vốn húc bằng sừng.
    // Nhưng Biệt Đội cũng có một con mang mã 'rook' mà là con khác hẳn (Con Ngồi, mù, bò dưới
    // đất, đánh bằng tay), và nó bị câu này cắt mất cặp tay theo. Hai bảng quái dùng chung một
    // hàm vẽ thì điều kiện phải hỏi bảng, không hỏi tên.
    const coTay = !d.noMelee && !d.noArms && (d.dmg || 0) > 0 &&
...
// CHÂN PHẢI ĐẢO KHI NGƯỜI ĐANG CHẠY, kể cả khi người ấy không thật sự đi đâu cả.
//
// `colFor()` bên sprites.js chọn khung chân theo QUÃNG ĐƯỜNG mà chính vật được truyền vào đã
// đi, và nó ghi mốc lên chính vật ấy (`_sx/_sy/_sd`). Trong đoạn phim thì toạ độ THẬT của cả
// tổ đứng yên — thứ chạy ngang qua màn hình là CHỖ VẼ — nên truyền thẳng nhân vật vào là quãng
// đường luôn bằng 0: bốn cái bóng trượt tới cửa xe trong tư thế đứng nghiêm.
//
// Cái vỏ này mang toạ độ VẼ, và nó phải là CÙNG MỘT vật qua các khung hình (giữ trên `a._ve`)
// chứ không phải một vật mới mỗi khung, không thì quãng đường lại về 0.
//
// NGƯỠNG 1,5: dưới ngưỡng thì ép về khung đứng. Ngồi trên thùng xe vẫn nhún 0,6 điểm ảnh mỗi
// khung — đủ để colFor() coi là "đang đi" và cho bốn người ngồi đó dậm chân tại chỗ suốt quãng
// đường. Một cú chạy thì nhanh gấp mấy lần chỗ ấy (đo được 4..5 điểm ảnh mỗi khung), nên một
// con số ở giữa tách được hai việc mà không cần ai truyền cờ vào.
const VE_CHAY = 1.5;
function voVe(a, x, y){
  const v = a._ve || (a._ve = { x, y, dir: a.dir });
  const dx = x - v.x, dy = y - v.y, d2 = dx*dx + dy*dy;
  if (d2 > VE_CHAY*VE_CHAY) v.dir = Math.atan2(dy, dx);   // chạy thì quay mặt về hướng chạy
  v.x = x; v.y = y;
  if (d2 <= VE_CHAY*VE_CHAY){ v._sx = x; v._sy = y; }     // đứng: ép colFor() về khung giữa
  v.hurt = a.hurt; v.charId = a.charId; v.id = a.id;
  return v;
}
function drawPlayer(c){
  ...
  const skin = window.REPO_SKIN && REPO_SKIN.crew(c, S.cut ? voVe(p, at.x, at.y) : p, true);
...
// con còn đúng một vệt ở mép trái.
//
// Không rộng gấp đôi như bản đầu nữa: từ nay ô này nằm CẠNH chân dung chứ không thay chỗ nó
// (chủ dự án, 2026-09-03: "gif nên nằm kế bên hình chứ đừng thay thế"), nên hai ô cộng lại phải
// còn chừa đủ chỗ cho chữ trên màn dọc của điện thoại. 48 + 78 + 9 khe = 135px, còn lại là chữ.
const WIKI_ANIM_W = 78;
function wikiAnimCao(){
  const K = (window.REPO_SKIN && REPO_SKIN.cell) || { w:96, h:144 };
  return Math.round(K.h * WIKI_SCALE);
}
function wikiChieuO(ch){
  const w = WIKI_ANIM_W, h = wikiAnimCao();
  return '<canvas class="wk-art wk-skill" data-hue="' + (ch.hue || 45) + '" ' +
         'data-sk="' + wikiEsc(JSON.stringify({ id: ch.skill.id,
                                                radius: ch.skill.radius || 0,
                                                dur: ch.skill.dur || 0 })) + '" ' +
         'width="' + (w*2) + '" height="' + (h*2) + '" ' +
         'style="width:' + w + 'px;height:' + h + 'px"></canvas>';
}
// `def` chỉ truyền khi con quái KHÔNG có trong MONSTERS — tức là quái của Biệt Đội. Gửi kèm
// ngay trong thẻ thay vì tra lại lúc vẽ: vòng vẽ chỉ có cái canvas trong tay, không có ngữ cảnh
// nào để biết hàng này ra từ bảng nào.
function wikiSong(type, def, canh){
  const w = WIKI_ANIM_W, h = wikiAnimCao();
  return '<canvas class="wk-art wk-live" data-foe="' + wikiEsc(type) + '" ' +
         (def ? 'data-def="' + wikiEsc(JSON.stringify(def)) + '" ' : '') +
         (canh ? 'data-canh="' + wikiEsc(canh) + '" ' : '') +
         'width="' + (w*2) + '" height="' + (h*2) + '" ' +
         'style="width:' + w + 'px;height:' + h + 'px"></canvas>';
}
...
function wikiAnimStart(){
  ...
  const ve = (now) => {
    ...
    // LỆCH PHA từng ô. Mọi cảnh đều bắt đầu ở cùng một mốc, nên không lệch thì cả trang vung
    // tay cùng một nhịp — bảy con quái nhấp nháy đồng loạt đọc ra như một lỗi vẽ chứ không
    // như bảy con vật. Ảnh chụp lần đầu bắt đúng khoảnh khắc đó: bảy ô cùng loé viền đỏ.
    let thuTu = 0;
    for (const cv of oList){
      const type = cv.getAttribute('data-foe');
      let d = MONSTERS[type];
      const raw = cv.getAttribute('data-def');
      if (raw){ try { d = JSON.parse(raw); } catch(e){ d = null; } }
      if (!d) continue;
      const ten = cv.getAttribute('data-canh') || WIKI_CANH_CUA[type] || 'vung',
            canh = WIKI_CANH[ten] || WIKI_CANH.vung;
      const t = (giay + (thuTu++) * 0.43) % canh.chuKy;
      // Một đối tượng DÙNG LẠI cho mỗi ô, không nặn mới mỗi khung. sprites.js đếm khung bước
      // chân theo QUÃNG ĐƯỜNG con quái đã đi (colFor lưu `_sx/_sy` ngay trên đối tượng), nên
      // một đối tượng mới toanh mỗi khung thì lần nào cũng 'vừa đứng yên' và chân không bao giờ
      // đảo. Giữ lại đối tượng là Bom con đi tới và Kẻ húc lao qua có bước chân thật.
      const m = cv._wkM || (cv._wkM = {});
      ...
      // Cỡ vẽ theo CHIỀU CAO ô, rồi mỗi cảnh kéo nhỏ thêm nếu nó cần chỗ đi lại. Kẻ húc lao
    ...
    for (const cv of oChieu){
      ...
      veChieu(cv, sk, +cv.getAttribute('data-hue'), giay + (thuTuC++) * 0.37);
    }
    wikiRaf = requestAnimationFrame(ve);
  };
  wikiRaf = requestAnimationFrame(ve);
}
// Con mắt của cảnh Tượng: mở thì Tượng đứng chết, nhắm thì nó nhảy tới. Vẽ ở góc trên ô, ngoài
// hệ toạ độ thế giới, vì nó là một CÁI NHÃN chứ không phải một vật trong phòng.
function wikiVeMat(c, cv, k, t){
  const mo = t < 1.5;
  c.save(); c.setTransform(1,0,0,1,0,0);
  const x = cv.width*0.5, y = cv.height*0.16, r = cv.height*0.075;
  c.strokeStyle = mo ? 'rgba(255,225,160,0.95)' : 'rgba(150,160,175,0.75)';
  c.lineWidth = Math.max(1.4, cv.height*0.016);
  c.beginPath();
  if (mo){
    c.moveTo(x-r*1.6, y); c.quadraticCurveTo(x, y-r*1.5, x+r*1.6, y);
    c.quadraticCurveTo(x, y+r*1.5, x-r*1.6, y); c.stroke();
    c.fillStyle = 'rgba(255,225,160,0.95)';
    c.beginPath(); c.arc(x, y, r*0.55, 0, Math.PI*2); c.fill();
  } else {
    c.moveTo(x-r*1.6, y); c.quadraticCurveTo(x, y+r*1.1, x+r*1.6, y); c.stroke();
  }
  c.restore();
}
function wikiAnimStop(){ if (wikiRaf) cancelAnimationFrame(wikiRaf); wikiRaf = 0; }
// Ô hình của MỘT CHIÊU.
//
// Bản đầu vẽ CHUNG một sơ đồ cho cả mười bốn chiêu: lưới ô, một vòng nở ra tới đúng tầm, một
// người ở tâm. Nó trả lời được đúng một câu — 'rộng chừng nào' — mà đó lại là câu mấy con số
// ngay bên cạnh đã trả lời rồi. Chói Loà, Vòng Hồi và Mở Toang khác nhau hoàn toàn về việc
// chúng LÀM GÌ, mà ba ô nhìn y hệt nhau. Chủ dự án nói đúng: "gif của player skill chưa rõ
// ràng".
//
// Nay mỗi chiêu một hình riêng, và hình đó diễn đúng cái ĐỘNG TỪ của nó: Chói Loà làm quái
// đứng hình, Kéo Đồ hút đồ về, Lồng Sắt dựng song chắn, Chớp là biến mất rồi hiện ra chỗ khác.
// Vòng tầm vẫn còn ở mấy chiêu có tầm, nhưng nó lùi xuống làm NỀN chứ không còn là nội dung.
//
// Vẫn KHÔNG dựng lại hiệu ứng thật của game — khác hẳn ô con quái, vốn gọi thẳng drawFoeOne.
// Mỗi chiêu là một đoạn mã ăn sâu vào trạng thái ván đấu (Chói Loà đọc danh sách quái, Mở Toang
// đọc danh sách cửa, Kéo Về đọc ai đang gục), dựng lại chúng trong một ô 78px là viết bản thứ
// hai của mười bốn thứ, và bản thứ hai thì sẽ lệch bản thứ nhất. Đây là SƠ ĐỒ, và sơ đồ thì
// được phép đơn giản hơn thứ nó mô tả — miễn là nó không nói sai.
function skNguoi(c, x, y, a){
  c.globalAlpha = a;
  c.fillStyle = 'rgba(226,232,238,0.94)';
  c.beginPath(); c.ellipse(x, y+0.10, 0.20, 0.30, 0, 0, Math.PI*2); c.fill();
  c.beginPath(); c.arc(x, y-0.26, 0.17, 0, Math.PI*2); c.fill();
  c.globalAlpha = 1;
}
// Một con quái con, đủ để đọc ra 'thứ này là kẻ địch'. `kieu` là chuyện đang xảy ra với nó.
function skQuai(c, x, y, kieu){
  c.fillStyle = kieu === 'dong' ? 'rgba(150,205,235,0.95)'
              : kieu === 'mo'   ? 'rgba(190,150,150,0.45)'
                                : 'rgba(226,138,118,0.95)';
  c.beginPath();
  c.moveTo(x-0.23, y+0.25); c.lineTo(x-0.18, y-0.20); c.lineTo(x, y-0.31);
  c.lineTo(x+0.18, y-0.20); c.lineTo(x+0.23, y+0.25); c.closePath(); c.fill();
  if (kieu === 'choang'){                            // đứng hình: sao quay trên đầu
    // KHÁC hẳn nét băng của 'dong', và cố ý. Chói Loà làm quái ĐỨNG HÌNH, Đóng Băng làm quái
    // ĐÓNG BĂNG — hai chiêu khác nhau, hai ô hình phải khác nhau, nếu không thì việc tách mười
    // bốn sơ đồ ra chẳng giải quyết được gì cho đúng hai ô này.
    c.fillStyle = 'rgba(255,230,150,0.95)';
    for (let q = 0; q < 3; q++){ const a = q*2.1;
      c.beginPath(); c.arc(x + Math.cos(a)*0.24, y - 0.48 + Math.sin(a)*0.09, 0.07, 0, Math.PI*2);
      c.fill(); }
  } else if (kieu === 'dong'){
    c.strokeStyle = 'rgba(220,245,255,0.9)'; c.lineWidth = 0.06;
    for (let q = 0; q < 3; q++){ const a = q*Math.PI/3;
      c.beginPath();
      c.moveTo(x-Math.cos(a)*0.34, y-Math.sin(a)*0.34);
      c.lineTo(x+Math.cos(a)*0.34, y+Math.sin(a)*0.34); c.stroke(); }
  } else if (kieu === 'hoi'){                       // mất dấu: dấu hỏi trên đầu
    c.fillStyle = 'rgba(235,235,205,0.9)';
    c.fillRect(x-0.05, y-0.64, 0.10, 0.20); c.fillRect(x-0.05, y-0.38, 0.10, 0.08);
  } else if (kieu === 'thay'){                      // thấy người: dấu than
    c.fillStyle = 'rgba(255,120,100,0.95)';
    c.fillRect(x-0.05, y-0.68, 0.10, 0.25); c.fillRect(x-0.05, y-0.37, 0.10, 0.09);
  }
}
function skMui(c, x1, y1, x2, y2, mau){            // mũi tên: hướng đi của thứ đang chuyển
  c.strokeStyle = mau; c.lineWidth = 0.09; c.lineCap = 'round';
  c.beginPath(); c.moveTo(x1,y1); c.lineTo(x2,y2); c.stroke();
  const g = Math.atan2(y2-y1, x2-x1);
  c.beginPath();
  c.moveTo(x2, y2); c.lineTo(x2 - Math.cos(g-0.5)*0.22, y2 - Math.sin(g-0.5)*0.22);
  c.moveTo(x2, y2); c.lineTo(x2 - Math.cos(g+0.5)*0.22, y2 - Math.sin(g+0.5)*0.22);
  c.stroke(); c.lineCap = 'butt';
}
let SK_CAT = false;                                // vòng tầm đang bị khung cắt bớt?
function skVong(c, r, vien, nen){                  // vòng tầm, làm nền
  if (nen){ c.fillStyle = nen; c.beginPath(); c.arc(0,0,r,0,Math.PI*2); c.fill(); }
  c.strokeStyle = vien; c.lineWidth = 0.09;
  if (SK_CAT) c.setLineDash([0.22, 0.16]);
  c.beginPath(); c.arc(0,0,r,0,Math.PI*2); c.stroke();
  c.setLineDash([]);
}
// Mười bốn động từ. `u` chạy 0..1 trong một vòng lặp, `R` là tầm đã kẹp vừa khung, `M(a)` là
// màu riêng của nhân vật đó. Trả về số = độ mờ của người; trả về {dich} = người đứng chỗ khác.
const SK_HINH = {
  flash(c, u, R, M){                               // Chói Loà: quái đứng hình
    const r = R * (1 - Math.pow(1 - clamp(u/0.3,0,1), 3));
    skVong(c, r, M(0.8), M(0.10));
    if (u < 0.3){
      c.strokeStyle = M(1 - u/0.3); c.lineWidth = 0.1;
      for (let q = 0; q < 8; q++){ const a = q*Math.PI/4;
        c.beginPath();
        c.moveTo(Math.cos(a)*0.5, Math.sin(a)*0.5);
        c.lineTo(Math.cos(a)*r*0.92, Math.sin(a)*r*0.92); c.stroke(); }
    }
    for (let q = 0; q < 3; q++){ const a = q*2.1 + 0.4;
      skQuai(c, Math.cos(a)*R*0.62, Math.sin(a)*R*0.62, u > 0.3 ? 'choang' : 'thay'); }
  },
  healring(c, u, R, M){                            // Vòng Hồi: dấu cộng bay lên trong vòng
    skVong(c, R, M(0.75), M(0.12));
    for (let q = 0; q < 4; q++){
      const ph = (u*1.6 + q*0.25) % 1, x = (q-1.5)*0.44, y = 0.45 - ph*1.55;
      c.globalAlpha = 1 - ph; c.fillStyle = M(0.95);
      c.fillRect(x-0.06, y-0.20, 0.12, 0.40); c.fillRect(x-0.20, y-0.06, 0.40, 0.12);
    }
    c.globalAlpha = 1;
  },
  gong(c, u, R, M){                                // Gồng: vác nặng mà vẫn đi nhanh
    const ph = 0.5 + 0.5*Math.sin(u*Math.PI*4);
    c.strokeStyle = M(0.30 + ph*0.5); c.lineWidth = 0.1;
    c.beginPath(); c.arc(0, 0, 0.68 + ph*0.14, 0, Math.PI*2); c.stroke();
    c.fillStyle = 'rgba(196,158,108,0.95)'; c.fillRect(-0.32, -0.92, 0.64, 0.40);
    c.strokeStyle = M(0.7); c.lineWidth = 0.08;
    for (let q = 0; q < 3; q++){ const y = -0.3 + q*0.32;
      c.beginPath(); c.moveTo(-0.95 - ph*0.32, y); c.lineTo(-0.5, y); c.stroke(); }
  },
  unlock(c, u, R, M){                              // Mở Toang: cửa bung hai cánh
    skVong(c, R, M(0.65), M(0.07));
    const mo = clamp((u-0.25)/0.3, 0, 1);
    for (let q = 0; q < 3; q++){ const a = q*2.1 + 0.6;
      c.save();
      c.translate(Math.cos(a)*R*0.72, Math.sin(a)*R*0.72); c.rotate(a);
      c.fillStyle = 'rgba(178,138,94,0.95)';
      c.save(); c.rotate(-mo*1.15); c.fillRect(0, -0.44, 0.13, 0.44); c.restore();
      c.save(); c.rotate( mo*1.15); c.fillRect(0,  0,    0.13, 0.44); c.restore();
      c.restore(); }
  },
  vanish(c, u, R, M){                              // Tàng Hình: quái mất dấu
    for (let q = 0; q < 2; q++) skQuai(c, (q?1.2:-1.2), -0.1, u > 0.35 ? 'hoi' : 'thay');
    c.strokeStyle = M(0.8); c.lineWidth = 0.08; c.setLineDash([0.14, 0.14]);
    c.beginPath(); c.ellipse(0, 0.02, 0.31, 0.47, 0, 0, Math.PI*2); c.stroke();
    c.setLineDash([]);
    return u > 0.35 ? 0.10 : 1 - u/0.35*0.9;
  },
  shock(c, u, R, M){                               // Xung Chấn: một phát, hất văng
    const r = R * clamp(u/0.28, 0, 1), tan = 1 - clamp((u-0.28)/0.4, 0, 1);
    skVong(c, r, M(0.9*tan), null);
    for (let q = 0; q < 3; q++){
      const a = q*2.1 + 0.4, d = 0.7 + clamp(u/0.42, 0, 1)*R*0.62;
      if (u < 0.55) skMui(c, Math.cos(a)*0.8, Math.sin(a)*0.8,
                          Math.cos(a)*(d+0.42), Math.sin(a)*(d+0.42), M(0.75));
      skQuai(c, Math.cos(a)*d, Math.sin(a)*d, 'thuong');
    }
  },
  decoy(c, u, R, M){                               // Mồi Nhử: quái quay sang chỗ khác
    const dx = 1.55;
    c.fillStyle = M(0.22);
    c.beginPath(); c.arc(dx, 0, 0.72 + 0.13*Math.sin(u*Math.PI*4), 0, Math.PI*2); c.fill();
    c.strokeStyle = M(0.9); c.lineWidth = 0.09; c.setLineDash([0.12, 0.12]);
    c.beginPath(); c.ellipse(dx, 0.02, 0.29, 0.45, 0, 0, Math.PI*2); c.stroke();
    c.setLineDash([]);
    for (let q = 0; q < 2; q++){
      const y = q ? 1.05 : -1.05, x = -1.5 + clamp(u/0.7, 0, 1)*2.0;
      skMui(c, x + 0.34, y*0.92, dx - 0.5, y*0.42, M(0.6));
      skQuai(c, x, y, 'thuong');
    }
    return { dich: -1.5 };
  },
  rescue(c, u, R, M){                              // Kéo Về: lôi người gục về chỗ mình
    const k = clamp((u-0.15)/0.55, 0, 1), x = 1.9*(1-k) + 0.55;
    c.fillStyle = 'rgba(206,148,148,0.9)';
    c.beginPath(); c.ellipse(x, 0.42, 0.36, 0.17, 0, 0, Math.PI*2); c.fill();
    c.beginPath(); c.arc(x + 0.42, 0.34, 0.15, 0, Math.PI*2); c.fill();
    skMui(c, x + 0.05, 0.05, 0.45, -0.02, M(0.85));
  },
  cage(c, u, R, M){                                // Lồng Sắt: song chắn mọc lên
    const k = clamp(u/0.3, 0, 1);
    c.strokeStyle = M(0.85); c.lineWidth = 0.1;
    for (let q = 0; q < 8; q++){ const a = q*Math.PI/4;
      c.beginPath();
      c.moveTo(Math.cos(a)*1.2, Math.sin(a)*1.2 - 0.58*k);
      c.lineTo(Math.cos(a)*1.2, Math.sin(a)*1.2 + 0.34*k); c.stroke(); }
    skQuai(c, -1.95, 0, 'thuong');
    if (k > 0.85) skMui(c, -1.6, 0, -1.34, 0, M(0.5));
  },
  blink(c, u, R, M){                               // Chớp: biến mất, hiện ra chỗ khác
    const sang = u < 0.5 ? -1.5 : 1.5;
    c.strokeStyle = M(0.5); c.lineWidth = 0.08; c.setLineDash([0.16, 0.16]);
    c.beginPath(); c.moveTo(-1.5, 0); c.lineTo(1.5, 0); c.stroke(); c.setLineDash([]);
    c.strokeStyle = M(0.85); c.lineWidth = 0.09;
    c.beginPath(); c.arc(-sang, 0, 0.32 + (u % 0.5)*0.9, 0, Math.PI*2); c.stroke();
    return { dich: sang };
  },
  reveal(c, u, R, M){                              // Thấu Thị: thấy cả thứ sau tường
    c.fillStyle = 'rgba(118,120,132,0.55)'; c.fillRect(0.6, -1.7, 0.4, 3.4);
    const hien = clamp((u-0.2)/0.35, 0, 1);
    c.globalAlpha = hien;
    for (let q = 0; q < 2; q++){ const y = q ? 0.8 : -0.8;
      c.strokeStyle = M(0.9); c.lineWidth = 0.08;
      c.beginPath(); c.arc(1.62, y, 0.52, 0, Math.PI*2); c.stroke();
      skQuai(c, 1.62, y, 'thuong'); }
    c.globalAlpha = 1;
  },
  freeze(c, u, R, M){                              // Đóng Băng: quái cứng đờ
    skVong(c, R, 'rgba(170,225,250,0.85)', 'rgba(150,210,245,0.13)');
    for (let q = 0; q < 3; q++){ const a = q*2.1 + 0.4;
      skQuai(c, Math.cos(a)*R*0.6, Math.sin(a)*R*0.6, u > 0.3 ? 'dong' : 'thuong'); }
  },
  pull(c, u, R, M){                                // Kéo Đồ: đồ tự bay về
    skVong(c, R, M(0.5), null);
    for (let q = 0; q < 4; q++){
      const a = q*Math.PI/2 + 0.5, ph = (u*1.3 + q*0.2) % 1, d = R*(1-ph)*0.82 + 0.55;
      if (ph < 0.78) skMui(c, Math.cos(a)*d, Math.sin(a)*d,
                           Math.cos(a)*(d-0.5), Math.sin(a)*(d-0.5), M(0.45));
      c.fillStyle = 'rgba(230,198,120,' + (0.35 + 0.6*ph).toFixed(2) + ')';
      c.fillRect(Math.cos(a)*d - 0.16, Math.sin(a)*d - 0.16, 0.32, 0.32);
    }
  },
  angel(c, u, R, M){                               // Bất Tử: đòn đánh dội ngược ra
    const ph = 0.5 + 0.5*Math.sin(u*Math.PI*4);
    c.strokeStyle = M(0.5 + ph*0.45); c.lineWidth = 0.13;
    c.beginPath(); c.arc(0, 0, 0.88, 0, Math.PI*2); c.stroke();
    c.strokeStyle = 'rgba(255,240,190,0.95)'; c.lineWidth = 0.09;
    c.beginPath(); c.ellipse(0, -0.66, 0.29, 0.10, 0, 0, Math.PI*2); c.stroke();
    const den = clamp(u/0.45, 0, 1);
    skQuai(c, -1.85 + den*0.72, 0, 'thuong');
    if (u > 0.45) skMui(c, -1.0, -0.12, -1.6, -0.55, M(0.85));
  }
};
function veChieu(cv, sk, hue, t){
  ...
  // Khung nhìn KHÔNG bám theo tầm thật. Đây là chỗ bản trước hỏng: nó zoom ra đủ để vòng tầm
  // lọt trọn, nên Mồi Nhử (tầm 16 ô) và Chói Loà (5,5 ô) thu người với quái xuống còn ba bốn
  // điểm ảnh — đúng nghĩa 'chưa rõ ràng'. Một ô 78px chỉ chứa được chừng ba ô rưỡi mà vẫn còn
  // đọc được hình.
  //
  // Nên: giữ khung gần, và khi vòng tầm KHÔNG lọt thì nói thẳng ra bằng nét đứt cộng ba mũi
  // chevron chỉ ra ngoài, thay vì lặng lẽ vẽ một vòng nhỏ hơn sự thật. Con số 'TẦM 16 ô' nằm
  // ngay bên cạnh mới là chỗ nói đúng độ lớn; ô hình lo phần 'nó làm gì'.
  const oNhin = clamp(R0 + 0.7, 2.7, 3.6);
  ...
  c.stroke();
  const chuKy = 3.0, u = (t % chuKy) / chuKy;
  const M = (a) => 'hsla(' + (hue||45) + ',80%,66%,' + a + ')';
  const Rve = Math.min(R0 || 2.3, oNhin - 0.5);
  SK_CAT = R0 > Rve + 0.05;
  const ve = SK_HINH[sk.id];
  let mo = 1, dich = 0;
  if (ve){
    const r = ve(c, u, Rve, M);
    if (typeof r === 'number') mo = r;
    else if (r && r.dich != null) dich = r.dich;
  } else if (R0 > 0){
    skVong(c, Rve * clamp(u/0.32, 0, 1), M(0.8), M(0.1));
  }
  skNguoi(c, dich, 0, mo);
  if (R0 > Rve + 0.05){                            // vòng bị cắt: ba chevron chỉ ra ngoài
    c.strokeStyle = M(0.55); c.lineWidth = 0.08;
    for (let q = 0; q < 3; q++){
      const a = -0.5 + q*0.5, r = Rve + 0.16;
      c.beginPath();
      c.moveTo(Math.cos(a)*r - Math.sin(a)*0.16, Math.sin(a)*r + Math.cos(a)*0.16);
      c.lineTo(Math.cos(a)*(r+0.22), Math.sin(a)*(r+0.22));
      c.lineTo(Math.cos(a)*r + Math.sin(a)*0.16, Math.sin(a)*r - Math.cos(a)*0.16);
      c.stroke();
    }
  }
  ...
  if (sk.dur){
    ...
    c.fillStyle = M(0.9); c.fillRect(6, y, w * clamp(u/0.85, 0, 1), 3);
  }
}
...
function wikiHtml(){
  ...
  if (SQd && SQd.CHARS){
    ...
    for (const ch of SQd.CHARS){
      // Chân dung TRƯỚC, sơ đồ chiêu SAU. Hàng này nói về một cái chiêu, nhưng người đọc tìm
      // hàng bằng KHUÔN MẶT — đó là thứ họ vừa nhìn ở màn chọn đội. Bỏ mặt đi thì mười bốn hàng
      // thành mười bốn ô đồ hoạ na ná nhau, không hàng nào neo vào ai.
      h += wikiRow(wikiFace(url('crewUrl', ch.id), { col:'#3a4450' }) + wikiChieuO(ch),
...
window.REPO = {
  ...
  CUT: { ARRIVE: CUT_ARRIVE, HIT: CUT_HIT, SKID: CUT_SKID,
         ...
         DEPART: CUT_DEPART, LEN: [CUT_LEN_DAU, CUT_LEN_CHAY, CUT_LEN_LECH],
         SHUT: CUT_DOOR_SHUT.slice(), OFF: CUT_DRIVE_OFF.slice() },
